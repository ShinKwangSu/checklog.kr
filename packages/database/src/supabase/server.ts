// =============================================================================
// checklog.kr MVP — Supabase 서버 클라이언트
// =============================================================================
//
// 두 종류의 클라이언트를 제공한다.
//
...
//    - 로그인 전에는 세션/고객이 없으므로 RLS-respecting 연결로는
//      accounts 행을 읽거나(이메일 조회) 새 account 를 insert 할 수 없다.
//    - accounts 테이블은 db-schema 설계상 RLS 미적용이며, password_hash 같은
//      민감 컬럼은 "서버에서만" SELECT 한다(클라이언트로 절대 노출 금지).
//    - service_role 키는 절대 클라이언트 번들에 포함되어선 안 된다.
//      이 파일은 'server-only' 로 보호한다.
//
// 주의: service_role 은 RLS 를 우회하므로, 멀티고객 데이터(workspaces 등)를
//       다루는 Server Action 에서는 반드시 코드 레벨에서 account_id 필터를
//       명시해야 한다(01_db_schema.md "account_id 접근 방법" 참조).
// =============================================================================
 ...
 * 인증(로그인/회원가입) 등 세션이 없는 컨텍스트에서 accounts 테이블 접근에 사용한다.
 *
