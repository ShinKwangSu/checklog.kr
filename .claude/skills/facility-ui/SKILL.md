---
name: facility-ui
description: checklog.kr apps/app 시설 관리 UI 구현 가이드. React Query 훅 + shadcn/ui + Tailwind CSS. 회원가입/로그인, 워크스페이스, 시설 타입, 시설 정보 CRUD 화면. UI Engineer 에이전트가 apps/app 화면 구현 시 반드시 이 스킬을 사용한다.
---
...
```typescript
// UI 컴포넌트 — @checklog/ui에서 import
import { Button, Card, CardContent, CardHeader, CardTitle } from '@checklog/ui'
import { Form, FormField, FormItem, FormLabel, FormControl, FormMessage } from '@checklog/ui'
import { Input, Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from '@checklog/ui'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@checklog/ui'
...
// 층수 변환 유틸
import { generateFloorOptions, floorToDisplay } from '@checklog/database'
```
...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from '@checklog/ui'
...
```typescript
import { generateFloorOptions } from '@checklog/database'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@checklog/ui'
