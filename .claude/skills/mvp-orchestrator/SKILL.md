...
- "슈퍼어드민", "고객 관리", "admin" → `apps/admin` 파이프라인
...
4. _workspace/app/02_auth_session.md — 세션 구조, accountId 접근 방법 요약
...
- nextjs-guide의 도메인 레이어드 아키텍처 구조를 반드시 따른다
- Supabase 클라이언트는 @checklog/database에서 import, repository에 주입
- 슈퍼어드민은 account_id 필터 없이 전체 데이터 접근 (의도된 동작)
- 변경 Action은 ActionResult 패턴, 조회 Action은 throw

산출물:
1. apps/admin/domain/account/ — types/queries/hooks/actions/service/repository/mapper
...
4. apps/admin/app/dashboard/accounts/page.tsx (고객 목록, prefetch + nuqs)
5. apps/admin/app/dashboard/accounts/[accountId]/page.tsx (고객 상세, prefetch)
...
apps/app 추가 검증:
- 데이터 격리: repository의 account_id 필터 + RLS 정책 교차 확인
...
- 데이터 노출: account_id 필터 없음 확인 (슈퍼어드민 전체 접근이 의도된 동작)
