---
name: db-schema
description: checklog.kr MVP Supabase 스키마 설계 가이드. accounts/workspaces/facility_types/facilities 테이블 DDL, RLS 정책, TypeScript 타입 생성. DB Architect 에이전트가 스키마 설계 작업 시 반드시 이 스킬을 사용한다. '스키마', '테이블 설계', 'RLS', '마이그레이션' 요청 시 트리거.
---
...
### accounts (마스터 계정/업체 — apps/app 사용자)
```sql
...
모든 테이블에 RLS를 활성화하고 account_id 기반으로 격리한다.

```sql
-- workspaces RLS
ALTER TABLE workspaces ENABLE ROW LEVEL SECURITY;

CREATE POLICY "account_isolation_workspaces"
ON workspaces
FOR ALL
...
CREATE POLICY "account_isolation_facility_types"
ON facility_types
FOR ALL
USING (account_id = current_setting('app.current_account_id')::UUID);
...
CREATE POLICY "account_isolation_facilities"
ON facilities
FOR ALL
USING (account_id = current_setting('app.current_account_id')::UUID);
```

> RLS 대신 Server Action에서 `account_id` WHERE 조건으로 직접 격리하는 방식도 가능하다. 두 방법 모두 적용하면 이중 안전장치가 된다.
...
- [ ] 모든 테이블에 UUID PK 사용
- [ ] workspaces, facility_types, facilities에 account_id FK 포함
- [ ] RLS 정책 또는 Server Action 레벨 account_id 필터 적용
