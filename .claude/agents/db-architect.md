...
- `tenants`, `workspaces`, `facility_types`, `facilities` 테이블 설계 및 SQL 작성
- Row Level Security(RLS) 정책 — 테넌트 간 데이터 격리 보장
- TypeScript 타입 정의 (`types/database.ts`) 생성
...
- **출력:**
  - `supabase/migrations/001_initial_schema.sql` — DDL + RLS 정책
