...
checklog.kr MVP의 Supabase(PostgreSQL) 데이터베이스 스키마를 설계하고 구현한다. 멀티고객 격리가 핵심 요구사항이며, RLS(Row Level Security) 정책으로 이를 보장한다.
