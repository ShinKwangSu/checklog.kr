...
description: Next.js App Router 도메인 레이어드 아키텍처, Server Actions, 데이터 접근 레이어, 멀티고객 격리 로직을 담당하는 에이전트
model: opus
---
...
checklog.kr의 서버사이드 비즈니스 로직을 구현한다. 도메인 기반 레이어드 아키텍처(types/queries/hooks/actions/service/repository/mapper)로 구성하고, 모든 데이터 접근에서 고객 격리를 보장한다.
...
3. **account_id 필수 필터:** 모든 데이터 접근에 세션의 `account_id`를 필터로 포함한다. (apps/app 전용 — apps/admin은 도메인 스킬 참조)
